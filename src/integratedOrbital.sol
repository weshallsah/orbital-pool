...
// NEW: Library to hold the struct for calling the math helper
library Helper {
...
// NEW: Interface to interact with the deployed Stylus Math Helper contract
interface IOrbitalMathHelper {
    ...
    function calculateRadius(uint256[] calldata reserves) external view returns (uint256);

    function calculateBoundaryTickS(uint256 r, uint256 k) external view returns (uint256);
}
...
contract orbitalPool {
    ...
    // NEW: Address of the deployed Stylus math helper contract
    ...
    // MODIFIED: Constructor now accepts the math helper's address
    constructor(IERC20[TOKENS_COUNT] memory _tokens, address _mathHelperAddress) {
        tokens = _tokens;
    ...
    function addLiquidity(uint256 k , uint256[TOKENS_COUNT] memory amounts) external {
        ...
        // MODIFIED: Offload radius calculation to the Stylus contract
    ...
    function _calculateSwapOutput(
        ...
        return mathHelper.solveTorusInvariant(
            dataForHelper,
            reservesVec,
            tokenIn,
            tokenOut,
    ...
    function _projectTickToBoundary(uint256 k, uint256[TOKENS_COUNT] memory) internal {
        Tick storage tick = ticks[k];
